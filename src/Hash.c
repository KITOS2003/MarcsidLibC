...
#ifdef HASH_TABLE_INCLUDE_IMPLEMENTATION
...
HashTable _HashTable_create(size_t table_size, size_t seed, alloc_t allocator,
                            realloc_t reallocator, free_t liberator);
void HashTable_destroy(HashTable *self);
void HashTable_add_entry(HashTable *self, const char *key, const void *data);
void HashTable_remove_entry(HashTable *self, char *key);
void *HashTable_get_entry(HashTable *self, const char *key);
...
static inline HashTableDarray HashTableDarray_create(size_t capacity,
                                                     alloc_t allocator,
                                                     ...
                                                     free_t liberator)
{
    ...
    size_t *index_stack =
        allocator(INITIAL_INDEX_STACK_CAPACITY * sizeof(size_t));
...
static inline void HashTableDarray_index_stack_push(HashTableDarray *self,
                                                    size_t index)
{
    self->index_stack_top += 1;
    size_t index_stack_n_elements =
        self->index_stack_top - self->index_stack + 1;
    if (index_stack_n_elements >= self->index_stack_capacity)
    {
        self->index_stack_capacity *= 2;
        self->index_stack = self->reallocator(
            self->index_stack, self->index_stack_capacity * sizeof(size_t));
...
static inline size_t HashTableDarray_index_stack_pop(HashTableDarray *self)
{
    ...
    if (index_stack_n_elements < self->index_stack_capacity / 4 &&
...
static inline HashTableEntry *HashTableDarray_push(HashTableDarray *self,
                                                   HashTableEntry *entry)
{
    ...
    {
        self->capacity *= 2;
        self->data = self->reallocator(self->data,
                                       self->capacity * sizeof(HashTableEntry));
    }
...
static inline void HashTableDarray_pop(HashTableDarray *self,
                                       HashTableEntry *entry)
{
                            ...
                            void (*liberator)(void *))
{
    ...
    HashTableDarray darray = HashTableDarray_create(
        hash_table_darray_initial_capacity, allocator, reallocator, liberator);
...
void HashTable_resize(HashTable *self, size_t new_size, size_t new_seed)
{
    ...
    for (uint32_t i = 0; i < *(darray->index_stack); i++)
    {
        if (darray->data[i].key != NULL)
        {
            ...
            size_t hash = MurmurHash2(darray->data[i].key,
                                      strlen(darray->data[i].key), self->seed) %
                          self->table_size;
            HashTableEntry **entry = &self->table[hash];
            while (*entry != NULL)
            {
...
#endif // #ifdef HASH_TABLE_INCLUDE_IMPLEMENTATION
